...
client.on('message', async (channel, tags, message, self) => {
    ...
    if (command === '!drip') {
        ...
        if (userInfo.address == '') return client.say(channel, 'You must call !create <validity> first.')
    ...
    if (command === '!execute') {
        ...
        executeWithRetries(murmurService, tx, 
            ...
            client.say(channel, `@${username}, transaction execution failed (timing). Attempting retry ${retries + 1} of ${process.env.MAX_RETRIES}`)
        }, () => {
            client.say(channel, `@${username}, maximum retry limit reached. Transaction failed.`)
        })
    }
